...
void task_entry(void*) noexcept {
  for (;;) {
    if (!hardware::ready(hardware::Driver::can) || !notifications_ready) {
      test::record(test::Test::can_receive, false);
      test::check_in(test::Test::can_receive);
      vTaskDelay(pdMS_TO_TICKS(250U));
      continue;
    }
    static_cast<void>(ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(250U)));
    for (;;) {
      const auto result = hardware::can().receive(drivers::no_wait);
      if (!result) {
        break;
      }
      const auto& frame = result.value();
      test::record(test::Test::can_receive,
...
bool create_can_rx_task() noexcept {
  if (!task_storage.create(task_entry, "can_rx", detail::io_priority)) {
    return false;
  }
  notifications_ready = hardware::connect_can_receive_event(
